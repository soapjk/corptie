...
final class WorktreeManagementNavigationTests: XCTestCase {
    ...
    func testWorktreeTabPushActionCoversLoadingFeedbackAndDisabledReasons() throws {
        ...
        XCTAssertFalse(view.contains(".alert(L10n(client.operationNoticeTitle)"))
        XCTAssertFalse(view.contains(".sheet(item: $client.cleanupResult)"))
        XCTAssertTrue(view.contains("worktree.passive-operation-feedback"))
        XCTAssertTrue(view.contains("worktree.passive-cleanup-result"))
        XCTAssertTrue(client.contains("func dismissCleanupResult()"))
