...
struct WorkCreateView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            HStack {
                Spacer()
                if let creationError { Text(creationError).font(.caption).foregroundStyle(.red) }
                Button(L10n("取消")) { dismiss() }.disabled(isCreating)
                Button(L10n(isCreating ? "正在创建…" : "创建")) {
                    if isFirstRun { createFirstWork() }
                    else if create() { dismiss() }
                }
                .keyboardShortcut(.defaultAction)
                .disabled(
                    isCreating || !EntityNamePolicy.isValid(name)
    ...
    private func createFirstWork() {
        ...
        Task { @MainActor in
            ...
            if let work = await client.createWork(
                id: creationId, name: name, description: detail.isEmpty ? nil : detail,
                workspaceId: workspaceId, contributorAgentIds: Array(contributorAgentIds)
            ) {
                onCreated(work)
                dismiss()
            } else {
                creationError = client.errorMessage ?? L10n("Work 创建失败，可重试。")
            }
        }
    }
