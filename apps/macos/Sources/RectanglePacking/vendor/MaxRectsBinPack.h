...
public:
	...
	// Corptie adapter: reserve an existing UI position using the same upstream
	// free-space splitter. No changes to scoring or packing heuristics.
	bool Reserve(const Rect &rect)
	{
		if (rect.x < 0 || rect.y < 0 || rect.width <= 0 || rect.height <= 0 ||
			rect.width > binWidth || rect.height > binHeight ||
			rect.x > binWidth - rect.width || rect.y > binHeight - rect.height)
			return false;
		for (const Rect &used : usedRectangles)
			if (!DisjointRectCollection::Disjoint(used, rect)) return false;
		PlaceRect(rect);
		return true;
	}
