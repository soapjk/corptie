...
const scope = { logicalSessionId: "logical:test", objectiveId: "objective:test", taskId: "task:test" };
...
test("B4 TelemetryCorrelator creates the only complete flat 12-field identity chain", () => {
  ...
  assert.deepEqual(Object.keys(correlation.identityChain).sort(), ["catalogVersion", "logicalSessionId", "objectiveId", "observationId", "providerBindingGeneration", "providerBindingId", "repositoryId", "runId", "sourceFingerprint", "toolsetVersion", "taskId", "worktreeId"].sort());
...
test("B2 Experiment/Report Store owns only the nine benchmark tables and enforces Session-scoped idempotency", () => {
  ...
  try {
    ...
    harness.store.binding.taskId = "task:other";
...
function payloadFor(type, c) {
  ...
  const identity = { objectiveId: scope.objectiveId, taskId: scope.taskId, logicalSessionId: scope.logicalSessionId, providerBindingId: bindingId, bindingGeneration: 7, repositoryId: "repository:test", worktreeId: "worktree:test", turnId: `turn:${request.attemptId}` };
