...
const scope = { logicalSessionId: "logical:test", workId: "work:test", taskId: "task:test" };
...
test("B4 TelemetryCorrelator creates the only complete flat 12-field identity chain", () => {
  ...
  assert.deepEqual(Object.keys(correlation.identityChain).sort(), ["catalogVersion", "logicalSessionId", "workId", "observationId", "providerBindingGeneration", "providerBindingId", "repositoryId", "runId", "sourceFingerprint", "toolsetVersion", "taskId", "worktreeId"].sort());
...
function payloadFor(type, c) {
  ...
  const identity = { workId: scope.workId, taskId: scope.taskId, logicalSessionId: scope.logicalSessionId, providerBindingId: bindingId, bindingGeneration: 7, repositoryId: "repository:test", worktreeId: "worktree:test", turnId: `turn:${request.attemptId}` };
