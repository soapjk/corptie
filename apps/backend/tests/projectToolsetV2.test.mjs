...
test("authoritative resolver binds Session, Startup, Snapshot and Toolset pointer without client identity", async () => {
  const authority = { logicalSessionId: "logical:one", objectiveId: "objective:one", workItemId: "work_item:one", repositoryId: "repository:aa", worktreeId: "worktree:bb" };
  const startupReceipt = { schemaVersion: 2, status: "ready", startupOperationId: "startup:one", logicalSessionId: authority.logicalSessionId, objectiveId: authority.objectiveId, workItemId: authority.workItemId, repositoryId: authority.repositoryId, worktreeId: authority.worktreeId, receiptHash: "2".repeat(64), resourceVersion: 3 };
  ...
  const context = await resolver.resolveProjectContext({ workingDirectory: "/project", authenticatedSession: { logicalSessionId: "logical:one", workItemId: "work_item:one", repositoryId: "client-spoof" } });
  ...
  const resolved = await resolver.resolveRunIsolationAuthorities({ workingDirectory: "/project", authenticatedSession: { logicalSessionId: "logical:one", workItemId: "work_item:one" }, toolsetReceiptId: receipt.receiptId });
  assert.deepEqual(resolved.toolsetValidationReceiptPointer, toolsetValidationReceiptPointer(receipt, authority));
  await assert.rejects(() => resolver.resolveProjectContext({ workingDirectory: "/project", authenticatedSession: { logicalSessionId: "logical:other", workItemId: "work_item:one" } }), { code: "TOOLSET_PERMISSION_DENIED" });
});
...
function makePorts() {
  ...
  return {
    ...
    startupBindingReceiptReader: { async get(ref) { return { schemaVersion: 2, status: "ready", startupOperationId: ref.startupOperationId, logicalSessionId: "logical:one", objectiveId: "objective:one", workItemId: "work_item:one", repositoryId: "repository:aa", worktreeId: "worktree:bb", receiptHash: ref.startupReceiptHash }; } },
...
function makeInput(projectRoot, declaration) { return { logicalSessionId: "logical:one", objectiveId: "objective:one", workItemId: "work_item:one", repositoryId: "repository:aa", worktreeId: "worktree:bb", projectRoot, idempotencyKey: "one", capabilityClass: "run_isolation_only", startupBindingRef: startupRef(), startupBindingReceiptRef: startupReceiptRef(), repositorySourceSnapshotReceiptRef: snapshotRef("1".repeat(64)), projectFacts: {}, declaration }; }
