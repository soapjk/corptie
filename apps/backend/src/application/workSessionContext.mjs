...
export function mergeWorkerSessionContexts({
  baseContext, directUserIntentContext = null, memoryContext = null, mentionContext = null,
  maxContextBytes = DEFAULT_MAX_TURN_CONTEXT_BYTES
} = {}) {
  if (!baseContext?.prompt) return null;
  const required = [baseContext, directUserIntentContext, mentionContext].filter((item) => item?.prompt);
