...
const PLATFORM_DOMAIN_COVERAGE = Object.freeze([
  ...
  { domain: "WorkItem", tool: "corptie_platform_work_items_manage", boundary: "ObjectiveApplicationService" },
...
const MUTATIONS_REQUIRING_IDEMPOTENCY = new Set(["create", "import", "publish", "reference", "create_work_item", "start_worker", "request"]);
...
export class PlatformOperationService {
  ...
  async execute(input = {}) {
    ...
    switch (input.tool) {
      ...
      case "corptie_platform_work_items_manage": result = await this.#workItems(args); break;
  ...
  #workItems(args) {
    assertKnown(args, ["action", "work_item_id", "target_work_item_id", "objective_id", "title", "dependency_type", "patch", "idempotency_key"]);
    switch (required(args.action, "action")) {
      case "list": return args.objective_id ? this.objectiveService.listWorkItemsByObjective(args.objective_id) : this.objectiveService.listWorkItems();
      case "get": return this.objectiveService.getWorkItem(required(args.work_item_id, "work_item_id"));
      case "create": return this.objectiveService.createWorkItem({ ...(args.patch ?? {}), objectiveId: required(args.objective_id, "objective_id"), title: required(args.title, "title") });
      case "update": return this.objectiveService.updateWorkItem(required(args.work_item_id, "work_item_id"), args.patch ?? {});
      case "delete": this.objectiveService.deleteWorkItem(required(args.work_item_id, "work_item_id")); return { deleted: true };
      case "dependencies": return this.objectiveService.listDependencies(required(args.work_item_id, "work_item_id"));
      case "add_dependency": return this.objectiveService.addDependency(required(args.work_item_id, "work_item_id"), required(args.target_work_item_id, "target_work_item_id"), args.dependency_type ?? "depends_on");
      case "remove_dependency": this.objectiveService.removeDependency(required(args.work_item_id, "work_item_id"), required(args.target_work_item_id, "target_work_item_id")); return { removed: true };
  ...
  async #sessions(args, binding) {
    ...
    switch (required(args.action, "action")) {
      ...
      case "create": return this.createSession({ agentId: required(args.agent_id, "agent_id"), providerId: required(args.provider_id, "provider_id"), workItemId: optional(args.work_item_id), title: optional(args.title), prompt: optional(args.prompt) });
  ...
  async #collaboration(args, binding) {
    assertKnown(args, ["action", "session_id", "objective_id", "agent_id", "work_item_id", "title", "description", "acceptance_criteria", "priority", "provider_id", "summary", "type", "max_iterations", "idempotency_key"]);
    ...
    switch (required(args.action, "action")) {
      ...
      case "create_work_item": {
        ...
        return this.store.runInTransaction(() => {
          const item = this.objectiveService.createWorkItem({ objectiveId, title: required(args.title, "title"), description: args.description ?? "", acceptanceCriteria: array(args.acceptance_criteria).join("\n"), priority: args.priority ?? "medium", mainAgentId: optional(args.agent_id) });
          this.store.db.run("UPDATE work_items SET created_by_session_id=?, idempotency_key=? WHERE id=?", [binding.actorSessionId, required(args.idempotency_key, "idempotency_key"), item.id]);
          return this.store.getWorkItem(item.id);
        });
      }
      case "start_worker": { const workItem = this.objectiveService.getWorkItem(required(args.work_item_id, "work_item_id")); const agentId = optional(args.agent_id) ?? workItem.main_agent_id; found(this.store.getAgent(required(agentId, "agent_id")), "AGENT_NOT_FOUND"); return this.createSession({ agentId, providerId: required(args.provider_id, "provider_id"), workItemId: workItem.id, title: optional(args.title), prompt: null }); }
      case "request": {
...
function receiptTarget(tool, args, result) {
  const type = tool.includes("artifacts") ? "Artifact" : tool.includes("collaboration") ? "Collaboration" : tool.includes("sessions") ? "Session" : tool.includes("work_items") ? "WorkItem" : tool.includes("objectives") ? "Objective" : tool.includes("agents") ? "Agent" : "Platform";
  const createdId = result?.artifact?.artifactId ?? result?.artifactId ?? result?.workItem?.id
  ...
  const requestedId = type === "Artifact"
    ...
    : type === "Session" ? args.session_id
      : type === "WorkItem" ? args.work_item_id
        : type === "Objective" ? args.objective_id
          : type === "Agent" ? args.agent_id
            : (args.session_id ?? args.work_item_id ?? args.objective_id);
