...
export class ProjectToolsetAuthorityResolver {
  ...
  async #authority(authenticatedSession, workingDirectory) {
    ...
    for (const key of ["logicalSessionId", "workId", "taskId", "repositoryId", "worktreeId"]) if (typeof authority?.[key] !== "string" || !authority[key]) fail("TOOLSET_PERMISSION_DENIED", "Session authority is incomplete.");
    ...
    return Object.freeze({ logicalSessionId: authority.logicalSessionId, workId: authority.workId, taskId: authority.taskId, repositoryId: authority.repositoryId, worktreeId: authority.worktreeId });
  }
}
...
function assertAuthorityIdentity(value, authority, code) { for (const key of ["logicalSessionId", "workId", "taskId", "repositoryId", "worktreeId"]) if (value?.[key] !== authority[key]) fail(code, `${key} does not match authenticated authority.`); }
