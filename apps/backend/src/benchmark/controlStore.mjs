...
export class BenchmarkControlStore {
  ...
  initialize() {
    ...
    db.run(`CREATE TABLE IF NOT EXISTS benchmark_experiments (
      record_id TEXT PRIMARY KEY, schema_version INTEGER NOT NULL, resource_version INTEGER NOT NULL,
      content_hash TEXT NOT NULL, created_by_session_id TEXT NOT NULL, created_at TEXT NOT NULL,
      work_id TEXT NOT NULL, task_id TEXT NOT NULL, idempotency_key TEXT NOT NULL,
    ...
    db.run("CREATE INDEX IF NOT EXISTS idx_benchmark_experiments_scope ON benchmark_experiments(work_id, task_id, created_at DESC)");
  ...
  createExperiment(scope, input) {
    ...
    this.#db().run(`INSERT INTO benchmark_experiments VALUES (?, 1, 1, ?, ?, ?, ?, ?, ?, ?, 'planned', ?)`, [recordId, contentHash(payload), scope.logicalSessionId, createdAt, scope.workId, scope.taskId, input.idempotencyKey, planHash, JSON.stringify(payload)]);
  ...
  getExperiment(scope, experimentId) {
    ...
    if (!row || row.work_id !== scope.workId || row.task_id !== scope.taskId || row.created_by_session_id !== scope.logicalSessionId) throw benchmarkError("BENCHMARK_EXPERIMENT_NOT_FOUND", "Experiment is not visible to this Session.", "store", { statusCode: 404 });
    return present(row);
  }

  listExperiments(scope) {
    return this.#db().all("SELECT * FROM benchmark_experiments WHERE work_id=? AND task_id=? AND created_by_session_id=? ORDER BY created_at DESC", [scope.workId, scope.taskId, scope.logicalSessionId], "BenchmarkControlStore.listExperiments").map(present);
  }
