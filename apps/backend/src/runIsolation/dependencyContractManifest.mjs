...
export const DEPENDENCY_CONTRACT_MANIFEST = Object.freeze({
  ...
  repositorySourceSnapshot: fixed({ owner: "repository_source_snapshot", receiptType: "RepositorySourceSnapshotReceipt", schemaVersion: 1, artifactId: "artifact:ee9b734f-799d-41b6-804f-9868697de511", version: 1, contentHash: "d706d828e697618d69a65b7c1fcae9fa95f41e2c70237cc60802a0e91f0c2e15" }),
  toolsetValidation: fixed({ owner: "project_toolset", receiptType: "ToolsetValidationReceipt", schemaVersion: 3, artifactId: "artifact:ed9a09d9-d2b1-4446-9a34-4ef491570ef3", version: 1, contentHash: "55d976162448d8519a3d7805502921ee3474fb665fd2b17d8582e3949ae98888" }),
...
export function verifyRepositorySourceSnapshotReceipt(receipt, reference, authority, manifest = DEPENDENCY_CONTRACT_MANIFEST) {
  ...
  for (const field of ["workId", "taskId", "logicalSessionId", "repositoryId", "worktreeId"]) if (Object.hasOwn(authority, field) && authority[field] !== receipt[field]) throw contractError("SOURCE_SNAPSHOT_IDENTITY_MISMATCH", `${field} differs from the authenticated Run identity.`);
  return receipt;
}

export function validateStartupBindingReceipt(receipt, reference, authority) {
  const required = ["startupOperationId","workId","taskId","logicalSessionId","repositoryId","worktreeId","canonicalWorktreePath","headIdentity","providerBindingId","bindingGeneration","sourceCommitOid","sourceTreeOid","baseRef","repositoryInventoryVersion","workspaceResourceVersion","resourceVersion","providerContextHash","toolContractHash","instructionSourcesHash","phaseTimestamps","compensation","error","receiptHash","schemaVersion","status"];
  ...
  for (const field of ["workId","taskId","logicalSessionId","repositoryId","worktreeId","providerBindingId","bindingGeneration","sourceCommitOid","sourceTreeOid","repositoryInventoryVersion","workspaceResourceVersion","resourceVersion"]) if (Object.hasOwn(authority, field) && authority[field] !== receipt[field]) throw contractError("STARTUP_BINDING_STALE", `${field} is stale against authoritative binding.`);
  return receipt;
}
