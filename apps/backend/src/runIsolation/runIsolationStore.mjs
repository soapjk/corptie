  ...
  createRun(input) {
    return this.transaction(() => {
      ...
      this.db.prepare(`INSERT INTO isolation_runs
        (run_id,mode,logical_session_id,work_item_id,repository_id,worktree_id,state,outcome,generation,root_relative_path,
         ...
         VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`).run(
        input.runId,input.mode,input.logicalSessionId,input.workItemId,input.repositoryId,input.worktreeId,"allocated",null,input.generation,
