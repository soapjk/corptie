...
export class RunIsolationService {
  ...
  async prepareRun(request,session){
    ...
    const created=this.store.createRun({runId,mode:request.mode,logicalSessionId:session.logicalSessionId,taskId:session.taskId,
    ...
    try{
      ...
      const context=Object.freeze({schemaVersion:3,runId,mode:request.mode,generation:1,logicalSessionId:session.logicalSessionId,taskId:session.taskId,
  ...
  #authorize(runId,session){const run=this.store.getRun(runId);if(!run)throw contractError("RUN_NOT_FOUND","Run not found.");if(run.logicalSessionId!==session?.logicalSessionId||run.taskId!==session?.taskId)throw contractError("RUN_UNAUTHORIZED","Authenticated Session does not own the Run.");return run}
...
function validatePrepareInput(r,s){if(!r||!["test","development"].includes(r.mode)||!r.idempotencyKey||!s?.logicalSessionId||!s?.taskId)throw contractError("RUN_CONTEXT_REQUIRED","prepareRun requires mode, idempotencyKey and authenticated Session.");const allowed=new Set(["mode","sourceAware","startupBindingReceiptRef","repositorySourceSnapshotReceiptRef","toolsetValidationReceiptPointer","toolsetRequired","idempotencyKey","testPlanRef","fixtureRef","quotaClass"]);for(const k of Object.keys(r))if(!allowed.has(k))throw contractError("RUN_CONTEXT_SCHEMA_UNSUPPORTED",`${k} is unsupported or owned by RunIsolationService.`)}
function assertAuthoritativeIdentity(r,s){for(const k of ["logicalSessionId","taskId","repositoryId","worktreeId"])if(Object.hasOwn(r,k)&&r[k]!==s[k])throw contractError("RUN_UNAUTHORIZED",`${k} differs from authoritative Session binding.`)}
