...
function collaborationRuntimeInstructions(agentId) {
  return [
    ...
    "Corptie programmatically creates and binds a WorkItem's Worktree before the first Turn. Continue in the active bound Workspace by default; never create, select, or switch a Worktree on your own. Use corptie_create_worktree or corptie_switch_workspace only when the direct user explicitly requests that exact Workspace operation; ordinary development, fixing, testing, committing, or inspection is not implicit authorization. corptie_list_workspaces remains available for user-requested inspection or startup-binding fault diagnosis. Never treat changing a command workdir or running cd as a logical workspace switch. A user-requested switch during a turn is applied only after that turn completes."
  ].join(" ");
}
