export function formatTrustedCollaborationEvent(envelope) {
  const peerContent = [
    ...
    line("来源 Work", envelope.message.envelope?.work?.sourceId ?? envelope.task.sourceWorkId),
    line("目标 Work", envelope.message.envelope?.work?.targetId ?? envelope.task.targetWorkId),
    line("目标 Session", envelope.task.recipientSessionId),
    line("路由版本", envelope.task.routingVersion),
    line("执行 Task", envelope.task.taskId),
    line("来源 Task", envelope.task.sourceTaskId),
...
export function formatTrustedChannelMessage(envelope) {
  ...
  return [
    ...
    line("来源 Work", senderContext.workId),
    line("来源 Task", senderContext.taskId),
    line("目标 Work", recipientContext.workId),
    line("目标 Task", recipientContext.taskId),
