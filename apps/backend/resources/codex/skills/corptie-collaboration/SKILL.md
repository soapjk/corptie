...
Session is the only executor, authorization context, Channel endpoint, and message sender or recipient. Agent, Objective, WorkItem, Workspace, Worktree, Provider, and Service are resources. Never target an Agent or WorkItem as a message recipient.

A Channel is a durable, bidirectional user authorization between two exact logical Sessions. It is not a Task, delegation, child WorkItem, acceptance workflow, or completion lifecycle. It may remain active across many messages, WorkItems, Provider bindings, and Workspace transitions.
...
3. If no target Session exists, supply the target Objective and Agent resource. One confirmation may authorize creation of the target WorkItem, target Worker Session, Channel, and first message. The Channel may activate only after the exact target Session is active.
...
The Channel identity contains only its two logical Sessions. WorkItem, Objective, Agent, Workspace, and Provider information belongs to each message's resource-context snapshot and never creates hierarchy or inherited authority.
...
- Decide locally whether a message can be answered directly, belongs in the current WorkItem, or justifies creating another independent WorkItem.
...
WorkItems are independent, equal resources. Creation provenance may record a direct user, Session, system operation, triggering message, or historical unknown source. Provenance is not collaboration, dependency, ownership, or parentage. Never create `delegated_subtask`, parent WorkItem, source WorkItem, or collaboration-relation fields from Channel communication.

Session-scoped WorkItem creation, start, cancellation, Artifact references, and acceptance remain separate product workflows. A Channel never completes or cancels a WorkItem.
