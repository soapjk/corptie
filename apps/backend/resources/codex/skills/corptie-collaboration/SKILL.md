...
3. If no target Session exists, creating a target Task and Worker Session requires the direct user's explicit confirmation. After confirmation, supply the target Work and Agent resources. The Channel may activate only after the exact target Session is active.
